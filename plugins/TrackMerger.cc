...
void TrackMerger::produce(edm::StreamID, edm::Event &evt, edm::EventSetup const &stp) const {
  ...
  for( unsigned int iTrk=0; iTrk<totalTracks; ++iTrk ) {
    ...
    // Consider only tracks close to at least one triggering muon
    float dzTrg = 0.0;
    if (trgMuons->size()>0) {
      bool skipTrack=true;
      for (const pat::Muon & mu: *trgMuons){
	// remove tracks inside trg muons jet
	if(reco::deltaR(trk, mu) < drTrg_Cleaning_ && drTrg_Cleaning_ >0) continue;
	// if dz is negative it is deactivated
	if((fabs(trk.vz() - mu.vz()) > dzTrg_cleaning_ && dzTrg_cleaning_ > 0)) continue;
	skipTrack=false;
	dzTrg = trk.vz() - mu.vz();
	break; // at least for one trg muon to pass this cuts
      }
      // if track is close to at least a triggering muon keep it
